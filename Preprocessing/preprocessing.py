...
class Load_Preprocess(object):
    def __init__(self):
        ...
        if configs["mock"] == True:
            ...
            dataset = load_data(
                os.path.abspath(__file__ + "/../../") + statics["data"]["mock"]
            )
        ...
        if configs["train"] == True:
            ...
            dataset = load_data(
                os.path.abspath(__file__ + "/../../") + statics["data"]["train"]
            )

            self.train_data = dataset
            self.train_features, self.train_targets = feature_target(dataset)
            self.train_features_num, self.train_mapping = numericalize(
                self.train_features
            )
        ...
        if configs["val"] == True:
            ...
            dataset = load_data(
                os.path.abspath(__file__ + "/../../") + statics["data"]["val"]
            )
        ...
        if configs["test"] == True:
            ...
            dataset = load_data(
                os.path.abspath(__file__ + "/../../") + statics["data"]["test"]
            )
        ...
        if configs["combine_train_val"]:
            ...
            if configs["load_pickle"]:
                ...
                self.train_val_features, self.train_val_targets = feature_target(
                    self.train_val
                )
                self.train_val_features_num, self.train_val_mapping = numericalize(
